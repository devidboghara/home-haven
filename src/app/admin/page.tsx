...
import { DollarSign, Home, UserCheck, TrendingUp, Sparkles, MapPin, Target, LayoutGrid } from "lucide-react";
import CountUp from "react-countup"; // Only if installed

export default function AdminDashboard() {
  const primaryStats = [
    { title: "Project Revenue Pipeline", val: 51273000, type: "currency", icon: <DollarSign size={20}/>, change: "+15.8%" },
    { title: "Active Listings", val: 28, type: "number", icon: <Home size={20}/>, change: "+5.2%" },
    { title: "Seller Leads Today", val: 65, type: "number", icon: <UserCheck size={20}/>, change: "+21.0%" },
  ];

  const barData = [50, 60, 45, 80, 70, 95, 40, 110, 65, 85, 55, 90, 75, 100]; // Fake Bar Heights for Activity Chart
  ...
  return (
    <div className="space-y-12">
          ...
          <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }} key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col justify-between group hover:border-blue-200 transition-all hover:translate-y-[-5px]">
            ...
            <div>
              ...
              <h4 className="text-3xl font-black text-slate-900 italic tracking-tighter leading-none">
                {item.type === "currency" ? "₹" : ""}
                <CountUp start={0} end={item.val} duration={1.5} separator="," />
                {item.type === "currency" ? " Cr+" : ""}
              </h4>
            </div>
          </motion.div>
        ))}
      ...
      {/* 📊 VISUAL DATA SECTION (Bar Chart Activity + Lead Workflow) */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        
        {/* LEADS ACTIVITY (Video Chart Style) */}
        <div className="lg:col-span-2 bg-white p-10 xl:p-12 rounded-[3rem] border border-slate-100 shadow-sm">
          ...
          {/* BAR CHART FAKE BARS */}
          <div className="h-60 flex items-end justify-between gap-1.5 lg:gap-2.5 px-4 relative">
        ...
        {/* PIPELINE LEAD WORKFLOW (Mini Kanban from Video) */}
        <div className="bg-white p-10 xl:p-12 rounded-[3rem] border border-slate-100 shadow-sm">
          <h3 className="text-sm font-black uppercase tracking-widest text-slate-900 mb-8 italic">Leads Pipeline Overview</h3>
          ...
          <button className="w-full mt-8 py-4 bg-slate-900 text-white rounded-[1rem] text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all">View All Deal States →</button>
        </div>
      </div>
    </div>
  );
